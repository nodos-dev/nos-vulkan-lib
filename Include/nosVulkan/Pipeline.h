...
struct hash<nos::vk::GraphicsPipelineKey>
{
	...
	{
		size_t hash = 0;
		nos::hash_combine(hash, k.OutputFormat, k.DepthFormat);
		return hash;
	}
};
} // namespace std
...
struct nosVulkan_API GraphicsPipeline : SharedFactory<GraphicsPipeline>, Pipeline
{
	...
	PerFormat CreateOrGet(GraphicsPipelineKey const& key);

private:
    std::unordered_map<GraphicsPipelineKey, PerFormat> Handles;
};
