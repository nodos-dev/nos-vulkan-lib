...
namespace mz::vk
{
    ...
    struct mzVulkan_API StandardPipeline : SharedFactory<StandardPipeline>, DeviceChild
    {
        ...
        StandardPipeline(Device* Vk, View<u8> src, VkSampler sampler = 0);

        StandardPipeline(Device* Vk, VkExtent2D extent, View<u8> src, VkSampler sampler = 0, std::vector<VkFormat> fmt = {});
        ~StandardPipeline();
