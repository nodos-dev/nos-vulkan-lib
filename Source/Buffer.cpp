	        ...
	        .AccessMask = VK_ACCESS_2_MEMORY_READ_BIT | VK_ACCESS_2_MEMORY_WRITE_BIT}, ElementType(info.ElementType)
{
	...
	if (auto* imported = info.Imported)
	{
		...
		NOSVK_ASSERT(Allocation->Import(device, Handle, *imported, bcInfos.MemProps));
	}
	else
	{
		...
		NOSVK_ASSERT(vmaCreateBufferWithAlignment(device->Allocator, &bcInfos.BufCreateInfo, &bcInfos.AllocCreateInfo, Alignment, &Handle, &Allocation->Handle, &Allocation->Info));
	}
	...
	if (bcInfos.ExtMemHandleType || info.Imported)
		NOSVK_ASSERT(Allocation->SetExternalMemoryHandleType(device, info.ExternalMemoryHandleType));
}
