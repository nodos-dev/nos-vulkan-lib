...
namespace mz::vk
{
    ...
    VertexShader* StandardPipeline::GetVS() const
    {
        if (!VS)
        {
    ...
    void StandardPipeline::ChangeTarget(rc<ImageView> Image)
    {
    ...
    void StandardPipeline::CreateWithImage(rc<ImageView> Image)
    {
    ...
    StandardPipeline::StandardPipeline(Device* Vk, View<u8> src, VkSampler sampler)
        : DeviceChild(Vk), PS(Shader::New(Vk, VK_SHADER_STAGE_FRAGMENT_BIT, src)), Layout(PipelineLayout::New(Vk, src, sampler))
    {
    }

    StandardPipeline::StandardPipeline(Device* Vk, VkExtent2D extent, View<u8> src, VkSampler sampler, std::vector<VkFormat> fmt)
        : DeviceChild(Vk), PS(Shader::New(Vk, VK_SHADER_STAGE_FRAGMENT_BIT, src)), Layout(PipelineLayout::New(Vk, src, sampler))
    {
...
#define ATT  ATT3, ATT3
    ...
    void StandardPipeline::BeginRendering(rc<CommandBuffer> Cmd, rc<ImageView> Image)
    {
        if (!Image)
        {
            assert(RenderTarget);
        ...
        framebufferInfo.pAttachments = &RenderTarget->Handle;
        framebufferInfo.width = (f32)extent.width;
        framebufferInfo.height = (f32)extent.height;
    ...
    bool StandardPipeline::BindResources(rc<CommandBuffer> Cmd, std::unordered_map<std::string, Binding::Type> const& resources)
    {
    ...
    void StandardPipeline::BindResources(rc<CommandBuffer> Cmd, std::map<u32, std::vector<Binding>> const& bindings)
    {
    ...
    void StandardPipeline::BindResources(rc<CommandBuffer> Cmd, std::map<u32, std::map<u32, Binding>> const& bindings)
    {
        Cmd->Callbacks.push_back([pipe = shared_from_this()]() { pipe->DescriptorSets.clear(); });
