...
DescriptorResourceInfo Binding::GetDescriptorInfo(VkDescriptorType type) const
{
    ...
    case VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE:
        assert(std::get<rc<Image>>(Resource)->Usage & VK_IMAGE_USAGE_SAMPLED_BIT);
        Info.Image.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        AccessFlags            = VK_ACCESS_SHADER_READ_BIT;
        break;
    case VK_DESCRIPTOR_TYPE_STORAGE_IMAGE:
        assert(std::get<rc<Image>>(Resource)->Usage & VK_IMAGE_USAGE_STORAGE_BIT);
        Info.Image.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
        AccessFlags            = VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT;
        break;
    case VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
        assert(std::get<rc<Image>>(Resource)->Usage & VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT);
        Info.Image.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
