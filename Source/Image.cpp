	  ...
	  }
{
	...
	if (auto* imported = createInfo.Imported)
	{
        ...
        if (NOS_VULKAN_SUCCEEDED(result))
            result = Allocation->Import(Vk, Handle, *imported, icInfos.MemProps);
	}
	else // Exported
	{
        ...
        result = vmaCreateImage(Vk->Allocator, &icInfos.ImgCreateInfo, &icInfos.AllocCreateInfo, &Handle, &Allocation->Handle, &Allocation->Info);
    }
	...
	if (NOS_VULKAN_SUCCEEDED(result) && icInfos.ExtMemHandleType)
		result = Allocation->SetExternalMemoryHandleType(Vk, icInfos.ExtMemHandleType);
